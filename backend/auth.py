...
def get_conversation(user_id: int, provider: str) -> dict | None:
    with db_session() as conn:
        row = conn.execute(
            """
    ...
    with db_session() as conn:
        conn.execute(
            ...
            INSERT INTO conversations (
              user_id, provider, agent_session_id, model, payload_json, updated_at
            ) VALUES (?, ?, ?, ?, ?, datetime('now'))
            ...
            """,
            (user_id, provider, agent_session_id or "", model or "", payload_json),
        )


def delete_conversation(user_id: int, provider: str) -> None:
    with db_session() as conn:
        conn.execute(
            "DELETE FROM conversations WHERE user_id = ? AND provider = ?",
            (user_id, provider),
        )
