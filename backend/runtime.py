...
class SessionManager:
    ...
    while True:
      ...
      if not self._pump_running(session):
        while idx < len(session.live_events):
          ...
          yield event
        if idx == 0:
          # Empty buffer — still close the SSE so refresh doesn't hang.
        ...
        elif start_idx >= len(session.live_events) and session.live_done:
          # Client already has every event (after past end); send terminal so it unblocks.
          yield sanitize_event({
            "type": "done",
            "session_id": session.session_id,
            "status": "finished",
  ...
  async def _pump_turn(
    ...
    try:
      ...
      try:
        while True:
          ...
          if isinstance(event, dict) and event.get("type") in {"text", "thinking", "planning"}:
            ...
            if text_has_secret(reply_so_far) or (isinstance(chunk, str) and text_has_secret(chunk)):
      ...
      if terminal:
        done_event["result"] = terminal
        if str(result.status).lower() in {"error", "failed"}:
          done_event["error"] = friendly_error(terminal)
      self._emit(session, done_event)
    except asyncio.CancelledError:
