...
def shell_title(cmd: str, status: str, description: str = "") -> str:
  """Cursor-like: title is Run / Running (or tool description); full command in detail."""
...
def tool_summary(name: str, args=None, result=None, status: str = "unknown") -> dict:
  ...
  if lowered == "tool" and cmd:
    ...
    if shell_looks_like_explore(cmd):
      token = cmd.split()[0] if cmd.split() else "command"
      token = token.rsplit("/", 1)[-1]
      return {
        "kind": "explore",
        "title": ("Running " if status == "running" else "Ran ") + token,
  ...
  if lowered in {"shell", "awaitshell", "bash", "terminal", "runterminalcmd", "runterminal"}:
    ...
    if shell_looks_like_explore(full_cmd or cmd):
      token = ((full_cmd or cmd).split() or ["command"])[0].rsplit("/", 1)[-1]
      return {
        "kind": "explore",
        "title": ("Running " if status == "running" else "Ran ") + token,
        "detail": shell_detail,
  ...
  return {
    ...
    "title": (
      ...
      if raw_name != "tool"
      else shell_title(cmd or detail or "command", status)
    ),
